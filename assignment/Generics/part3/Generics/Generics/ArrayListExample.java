...
public class ArrayListExample {
     ...
     */
	private static double total_area(ArrayList<? extends TwoDShape>exp_shape) {
		double result = 0;
		for (TwoDShape shape: exp_shape) {
     ...
     */
	private static double total_perimeter(ArrayList<? extends Rectangle>eg_shape) {
		double num = 0;
		for (Rectangle shape:eg_shape) {
			num = num + shape.perimeter();
		}
		return num;
	}
     ...
     */
	private static void describe_all(ArrayList<? extends GeometricShape> eg_shape) {
		for (GeometricShape shape: eg_shape) {
			shape.describe();
		}
		int total = eg_shape.size();
		System.out.print("Total number of shapes:" + total);
//		return total;
	}
     ...
     *      {@code new Sphere(0.0);          }
     */
	private static void add_empties(ArrayList<? super GeometricShape> eg_shape) {
		eg_shape.add(new Circle(0.0));
		eg_shape.add(new Cone(0.0,0.0));
		eg_shape.add(new Rectangle(0.0, 0.0));
		eg_shape.add(new Sphere(0.0));
	}
	private static <T extends GeometricShape<T>>ArrayList<T> supersize_list(ArrayList<T> lis) {
		// TODO Auto-generated method stub
		ArrayList<T> sup = new ArrayList<>();
		for (T shape: lis) {
			sup.add(shape.supersize());
		}
		return sup;
	}
    /**
